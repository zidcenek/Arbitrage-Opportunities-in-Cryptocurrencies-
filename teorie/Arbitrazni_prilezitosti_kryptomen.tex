...
Význam této práce spočívá v~analýze jednotlivých burz jako takových. Práce se~také zabývá otázkami, jak často se~reálně objevují arbitrážní příležitosti v~rámci jednotlivých burz, jak dlouho trvá, než tyto příležitosti zmizí, a~jestli je možné nacházet arbitrážní příležitosti v rámci vybrané burzy.
...
V tabulce \ref{table_combined_best} jsou ponechány pouze záznamy, u~kterých je možné v~průměru každý den vydělat více než 10 amerických dolarů. V~této sekci se~však nevěnuji všem, ale pouze pěti nejlukrativnějším trojúhelníkům (USDT/BTC/EOS, USDT/BTC/XRP, USDT/EOS/BNB, USDT/BTC/TRX, USDT/BTC/LTC).
...
Trojúhelník USDT/BTC/XRP je z~pohledu velikosti celkového potenciálního zisku druhý nejlepší. Trpí však podobnými problémy jako předchozí trojúhelník USDT/BTC/EOS. Denně se průměrně objevuje přes 25 arbitrážních příležitostí, avšak medián výskytu je stejně jako u předchozího trojúhelníku pouze 2. Tento vysoký rozdíl je taktéž způsoben velkým výkyvem z~12. A~13. března 2020 (viz graf \ref{occurences_arbitrage_USDTBTCXRP} a~tabulka \ref{USDTBTCXRP_stats}).

Trojúhelník USDT/BTC/XRP má také nadprůměrně dlouhé trvání arbitrážních příležitostí a to 0,43~s. Zajímavou hodntou je, že se na nejvýdělečnější arbitrážní příležitosti dalo zhodnotit zobchodovaný objem o 12~\%. 
...
Tento trojúhelník má velmi podobné vlastnosti jako oba výše zmiňované trojúhelníky. Průměrný počet denního výskytu arbitrážních příležitostí je zaokrouhleně 25 a medián se díky velkým výkyvům drží pouze na hodnotě 1 (viz \ref{USDTEOSBNB_stats}).
...
Trojúhelník USDT/BTC/LTC má velmi podobné vlastnosti jako všechny výše zmiňované. Má také o poznání vyšší denní průměr výskytu arbitrážních příležitostí než medián (viz \ref{USDTBTCLTC_stats}). Celkově má tento trojúhelník poměrně krátkou dobu trvání arbitrážních příležitostí, pouze 0,31~s. 
...
\begin{figure}\centering
	\includegraphics[width=1\textwidth]{images/best_triangles/occurences_arbitrage_USDTBTCLTC.png}
